...
function handleCollisions() {
  // 벽과의 충돌 처리
  if (ball.x + ball.radius > game.display.width || ball.x - ball.radius < 0)
    ball.transX *= -1;
  ...
  if (
    game.collision.check(
      ...
      game.draw.circle(ball.x, ball.y, ball.radius, ball.color)
    )
  ) {
    ball.transY *= -1; // 방향 전환
...
function handleInput() {
  game.event.getEvents().forEach((event) => {
    if (event.type === "Mouse_move") {
      paddle.x = Math.max(
        Math.min(
          event.position.x - paddle.width / 2,
          game.display.width - paddle.width
        ),
        0
      ); // 패들 위치 조정
    }
